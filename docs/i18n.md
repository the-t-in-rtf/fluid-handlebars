...
```handlebars
{{message-helper "my-variable-key"}}
```
...
```handlebars
{{message-helper "my-variable-key" subRecord}}
```
...
```handlebars
{{message-helper keys.variable subRecord}}
```
...
| ----------------- | ---------- | ----------- |
| `defaultLanguage` | `{String}` | The default language to use if no translation is available for a user's preferred locale/language. |
| `messages`        | `{Object}` | A map of message keys and string templates. Generally assembled from one or more files (see below). |


## Node Usage

By default, the `{{message-helper}}` helper is wired into [`gpii.handlebars.standaloneRenderer`](standaloneRenderer.md),
...
### `gpii.handlebars.inlineMessageBundlingMiddleware`
...
and follows the message bundling strategy outlined above.  The middleware supports the following configuration options:
...
language or locale suffix will be saved under a key that matches the value of `options.defaultLanguage` (see above).
...
against the language, and then against the language that corresponds to `options.defaultLanguage`.  So, a request for
...
{
    "fr_FR": {
        "localised-message": "bon jour"    
    },
    "fr": {
        "localised-message": "bon soir"
    },
    "en_US": {
        "localised-message": "good day",
        "localised-message-two": "thanks"
    },
    "en": {
       "localised-message": "good night",
       "localised-message-two": "cheers"
    }
}
```

If `options.defaultLanguage` is set to `en_US` and we request `fr_FR`, the compiled message bundle looks like:
...
`options.defaultLanguage` is set to `en` and we request `fr`, the compiled message bundle looks like:
...
For more complex setups, the [`gpii.handlebars.renderer.serverAware`](renderer.md) grade is more suitable.  This
