...
The `fluid.handlebars.helper.messageHelper` grade is designed to be wired into the [renderer](renderer.md) provided by
...
By default, the `{{messageHelper}}` helper is wired into [`fluid.handlebars.standaloneRenderer`](renderer.md),
...
To simplify the browser side usage, a piece of `fluid.express` middleware is provided that starts with the standard
...
`fluid.handlebars.i18n.messageBundleLoader` grade described above to populate this.

## Browser Usage

The base [`fluid.handlebars.renderer.standalone`](renderer.md) grade is adequate if you wish to include all message
bundles and handlebars templates as part of your grade definition.  See those docs for details.

For more complex setups, the [`fluid.handlebars.renderer.serverMessageAware`](renderer.md) grade is more suitable.  This
