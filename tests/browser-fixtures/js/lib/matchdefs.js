...
(function (fluid, jqUnit) {
    "use strict";
    fluid.registerNamespace("fluid.test.handlebars.browser");

    fluid.test.handlebars.browser.matchDefs = {
        standard: {
            markdown: {
     ...
     */
    fluid.test.handlebars.browser.elementMatches = function (message, element, pattern, invert) {
     ...
     * common renderer checks.  Calls `fluid.test.handlebars.elementMatches` (see above) for each element and match
     * definition.
     *
     ...
     *
     */
    fluid.test.handlebars.browser.sanityCheckElements = function (elements, matchDefs) {
            ...
            fluid.each(matchDefs, function (matchDef) {
                if (matchDef.selector) {
                    ...
                    fluid.each(subElements, function (subElement) {
                        fluid.test.handlebars.browser.elementMatches(matchDef.message + " (pass " + pass + ")", subElement, matchDef.pattern, matchDef.invert);
                        pass++;
                    });
                }
                else {
                    fluid.test.handlebars.browser.elementMatches(matchDef.message, element, matchDef.pattern, matchDef.invert);
                }
            });
        });
    };
     ...
     *
     */
    fluid.test.handlebars.browser.sanityCheckSelectors = function (selectors, matchDefs) {
        var elements = fluid.transform(fluid.makeArray(selectors), function (selector) {
            return $(selector);
        });
        fluid.test.handlebars.browser.sanityCheckElements(fluid.flatten(elements), matchDefs);
    };
})(fluid, jqUnit);
